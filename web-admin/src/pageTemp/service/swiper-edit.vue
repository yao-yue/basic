...
<script>
import GoodsServiceType from '@/constant/GoodsServiceType'
import { swiper, publicSysApi, service } from '@/agent'
...
export default {
  ...
  data() {
    return {
      id: null,
      currentPage:'',
      typeList:[{ name:'办公服务', value:'office' },{ name:'生活服务', value:'living' }],
      upLoadHeaders: {},
      upLoadUrl: "",
      imageUrl: "",
      tableData: [],
  ...
  methods: {
    ...
    // 返回
    handleGoBack() {
      this.$router.push({ path: "/service-swiper-list" });
    },
    // 选中关联
    selectBtn(obj){
    ...
    // 映射关系
    getType(value) {
      return GoodsServiceType.getName(value)
    },
