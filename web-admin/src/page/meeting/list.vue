<template>
  <div class="container">
    <div class="input_modal">
       <el-button type="primary" size="medium" @click="add('meeting-edit')">新增会议室</el-button>
    </div>
    <div class="input_modal">
      <div class="input-item">
        <span>会议室名称：</span>
        <el-input clearable v-model="name" placeholder="请输入会议室名称" class="proName" @keyup.enter.native="search"></el-input>
      </div>
      <div class="input-item">
        <span>城市：</span>
    ...
    <div class="table_con">
      <el-table :data="tableData" :border='true'>
        <el-table-column :formatter="formatter" prop="name" label="会议室名称"></el-table-column>
        <el-table-column :formatter="formatter" prop="address" label="会议室地址"></el-table-column>
        <el-table-column prop="coverUrl " label="会议室图片">
          <template slot-scope="obj">
            <img class="table_img" :src="obj.row.coverUrl">
          </template>
        </el-table-column>
        <el-table-column :formatter="formatter" prop="typeName" label="会议室类型"></el-table-column>
        <el-table-column :formatter="formatter" prop="cityName" label="城市"></el-table-column>
        <el-table-column :formatter="formatter" prop="price" label="价格(每小时)"></el-table-column>
        <el-table-column prop="" label="操作">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="add('meeting-detail',tableData[scope.$index].id)">查看</el-button>
            <el-button type="primary" size="mini" @click="add('meeting-edit',tableData[scope.$index].id)">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async getList() {
      ...
      let {data} = await meeting.getMeetList(param)
      if(data.status == 200) {
    ...
    deleteItem(id) {
      this.$confirm("确定删除?", "提示", {
        ...
        .then(async () => {
          let {data} = await meeting.delMeet(id);
          if(data.status == 200) {
            this.$message({
              type: "success",
              message: "删除成功!"
