    ...
    <div class="main">
      <el-form :model="campaign" :rules="rules" ref="campaign" label-width="150px">
        <el-form-item label="资讯标题：" prop="title" >
          <el-input v-model="campaign.title" placeholder="请输入资讯标题" />
        </el-form-item>

         <el-form-item label="描述：" prop="describe">
          <el-input v-model="campaign.describe" placeholder="请输入描述" />
        </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        describe: [{ required: true, message: "请输入库存", trigger: "blur" }],
  ...
  methods: {
    ...
    handleSave(formName) {
      this.$refs[formName].validate(async valid => {
        if (valid) {
          const param = { type: 'consult', ...this.campaign }
